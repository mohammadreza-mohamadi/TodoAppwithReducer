  ...
  return (
    <>
        {isEditing ? 
            ...
            <input type="text" value={todo.content}  onChange={(e)=>updateTodoContent(todo.id,e.target.value)} /><button onClick={()=>delTodo(todo.id)}>Del</button><button  onClick={()=>{setIsEditing(false)}}>Save</button></li>
            :
            <li><input type="checkbox" checked={todo.done ? true : false} onChange={()=>todoStatusHandler(todo.id)} />
